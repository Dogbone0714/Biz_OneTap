...
const CompanySearch = () => {
  ...
  const handleSearch = async (value) => {
    ...
    try {
      ...
      if (searchType === 'id') {
        url = `https://data.gcis.nat.gov.tw/od/data/api/5F64D864-61CB-4D0D-8AD9-492047CC1EA6?$format=json&$filter=${value} eq ${value}`;
      } else {
        const skip = (pagination.current - 1) * pagination.pageSize;
        url = `https://data.gcis.nat.gov.tw/od/data/api/6BBA2268-1367-4B42-9CCA-BC17499EBE8C?$format=json&$filter=${value} like ${value} and Company_Status eq '核准設立'&$skip=${skip}&$top=${pagination.pageSize}`;
      }
